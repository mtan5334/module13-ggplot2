...
# use dplyr's `sample_n(tablename, size)` function to get a random 1000 rows from the data set
...
# What do you see?
ggplot(data = diamonds.sample) #I DON'T SEE ANYTHING :(
...
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = price, color = clarity)) # MuLtIcOlOrS

# Draw the same plot as above, but for the entire `diamonds` data set. Note this may take
# a few seconds to generate.
ggplot(data = diamonds) +
  geom_point(mapping = aes(x = carat, y = price, color = clarity)) #IT'S A RAINBOW O_O

# Draw another scatter plot for `diamonds.sample` of price (y) by carat (x),
# but with all of the dots colored "blue".
# Hint: you'll need to set the color channel, not map a value to it!
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = price), color="blue") #BLUE DOTS :O

# Draw a scatter plot for `diamonds.sample` of `price` by `carat`, where each
# point has an aesthetic _shape_ based on the diamond's `cut`.
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = price, shape = cut)) #PRETTY SHAPES

# Draw a scatter plot for `diamonds.sample` of *`cut`* by `carat`, where each
# point has an aesthetic _size_ based on the diamond's *`price`*
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = price, size = price)) # MORE EXPENSIVE, MORE BIGGER DOTS!!!!! OMJJJJ

# Try coloring the above plot based on the diamond's price!
ggplot(data = diamonds.sample) +
  geom_point(mapping = aes(x = carat, y = price, size = price, color = price)) #IT'S A BEAUTIFUL GRADIENT ☆ω☆
...
ggplot(data = diamonds.sample) +
  geom_line(mapping = aes(x = carat, y = price, color = carat)) #TIL line plots aren't as pretty as dots (◕︿◕✿)

# That's kind of messy. Try using `smooth` geometry instead.
ggplot(data = diamonds.sample) +
  geom_smooth(mapping = aes(x = carat, y = price, color = carat)) # smooth line
