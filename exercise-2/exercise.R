# MODULE 13 Exercise 2: More ggplot2 Grammar
...
ggplot(data = diamonds) +
  stat_count(mapping = aes(x=cut)) #same graph but stat allows for statistical transformation to calculate the data
...
ggplot(data = diamonds) +
  stat_summary(mapping = aes(x = cut, y = depth),
              fun.ymin = min, fun.ymax = max, fun.y = median) #adds range to the dots?


## Position Adjustments

# Draw a bar chart of diamond data organized by cut, with each bar filled by clarity (coloring in bars)
